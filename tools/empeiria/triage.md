...
Where a row routes. The three-way test came from a dokimasia case study that
has since been retired from that tree; it is kept here because these two
projects are what still use it:

| class | means | goes to |
| --- | --- | --- |
| `proof` | it concerns whether a step can produce a proof | **dokimasia's** register, [its register](https://github.com/ajreynol/dokimasia/blob/main/docs/README.md#the-register) — another repository, so a person carries it |
