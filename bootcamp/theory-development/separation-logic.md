...
**Why do equal addresses conflict even when they store the same data?**
...
```sh
build-dev/bin/cvc5 -o post-asserts -o lemmas separation-logic.smt2
build-dev/bin/cvc5 -t im separation-logic.smt2
```

Follow a `SEP_` inference from a spatial assertion to constraints on its
heap label. Identify the disjointness requirement and the singleton domains;
then follow any set or arithmetic deductions that refute them. This is a
concrete example of one theory generating a problem for another.

For distinct addresses, enable model production and issue `(get-model)`.
Check that the heap entries contain both addresses and their data. Positive
heap decomposition does not by itself test the last-call handling of negative
spatial formulas; use the linked negative-spatial regression for that path.
