...
**Why does solving polynomial equations also require checking where their roots live?**
...
```sh
build-ff/bin/cvc5 -o post-asserts -o lemmas finite-fields.smt2
build-ff/bin/cvc5 -t im -t ff::model finite-fields.smt2
```

These commands use the [CoCoA build](../build.md#build-a-version-you-can-investigate).
Check that the square equation survives preprocessing. A refutation need not
construct a model, so `ff::model` may be quiet on the original input. Use the
`ff4` variant with `:produce-models` and `(get-value (x))` to observe a base-field
assignment, then verify its square modulo 5.

Compare with `--ff-field-polys` only as a separate experiment: it changes the
algebraic problem sent to the backend. Record the option with the output.
A missing-backend error is a build diagnosis and supplies no evidence about
root search.
...
use the inverse witness described in the representation section, whose value
...
In the GB route, the encoder scans the accumulated
